"""Phase 1 — first real ingest.

Reads data/fetch_log.jsonl (written by scripts/fetch.py) and ingests one
text_vec point per fetched asset. Exits with the count of new points written.

Run:  python scripts/phase1_log_ingest.py
"""
...
from revvec.ingestion.log import LogIngestor
...
log = logging.getLogger("phase1_log_ingest")
...
def main() -> int:
    ...
    with VectorAIClient(config.ACTIAN_URL) as client:
        client.connect()  # SDK requires explicit connect (not auto on context manager)
        memory = MemoryAgent(client)
        memory.ensure_ready()

        embedder = get_embedder()
        dedup = DedupStore(config.REVVEC_DATA / "dedup.sqlite")
        ingestor = LogIngestor(memory, embedder, dedup)
